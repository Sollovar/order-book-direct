    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans text-[13px] pb-6">
      ...
      <header className="flex items-center justify-between px-2 pt-4 pb-3">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 rounded-full border border-white/15 pl-1.5 pr-2 py-1">
            <span className="h-5 w-5 rounded-full bg-[#f0b90b] flex items-center justify-center text-black text-[10px] font-bold">
              ◆
            </span>
            <ChevronDown className="h-3 w-3 text-white/70" />
          </button>
          <button className="rounded-full border border-[#f5deb3]/40 text-[#f5deb3] px-4 py-1.5 text-[13px]">
            Connect
          </button>
          <button className="flex items-center gap-1 rounded-full border border-white/15 px-3 py-1.5 text-white/80">
            Old <span className="text-white/50">⇌</span>
          </button>
      ...
      <section className="mx-3 rounded-xl bg-[#111111] border border-white/5 p-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#f6465d] text-[13px] ml-1">-0.30%</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          {/* LEFT: order book */}
          <div>
            <div className="text-[11px] text-white/50 border-b border-dashed border-white/15 pb-1">
            ...
            {/* Mid price */}
            <div className="my-2 border-y border-dashed border-white/10 py-1.5">
              <div className="text-[#f6465d] text-[18px] font-medium leading-tight">
                66,007.4
              </div>
              <div className="text-white/50 text-[11px]">$66,008.9</div>
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
              <div className="grid grid-cols-2 gap-0.5">
                <div className="h-2 w-2 rounded-sm bg-[#0ecb81]" />
                <div className="h-2 w-3 bg-white/40 rounded-sm" />
                <div className="h-2 w-2 rounded-sm bg-[#f6465d]" />
                <div className="h-2 w-3 bg-white/40 rounded-sm" />
              </div>
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <button className="rounded-md bg-white/5 py-2 text-[13px]">Cross</button>
              <button className="rounded-md bg-white/5 py-2 text-[13px]">20x</button>
            </div>

            <button className="w-full rounded-md bg-white/5 py-2 flex items-center justify-center gap-1 text-[13px]">
              Limit <ChevronDown className="h-3 w-3" />
            </button>

            <div className="rounded-md bg-white/5 p-2 flex items-center justify-between">
              <div>
                <div className="text-[10px] text-white/50">Order price</div>
                <div className="text-[14px]">66015.6</div>
              </div>
              <div className="flex items-center gap-1">
                <span className="text-[11px] text-white/70 px-1.5">USDT</span>
                <span className="text-white/20">|</span>
                <span className="text-[11px] text-white/70 px-1.5">BBO</span>
              </div>
            </div>

            <div className="rounded-md bg-white/5 p-2 flex items-center justify-between">
              <div className="text-[12px] text-white/40">Size</div>
            ...
            <div className="py-2">
              <div className="relative h-[2px] bg-white/10 rounded-full">
                <div className="absolute left-0 top-1/2 -translate-y-1/2 h-3 w-3 rounded-full bg-white border border-white/60" />
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="absolute top-1/2 -translate-y-1/2 h-1 w-1 rounded-full bg-white/30"
                    style={{ left: `${i * 20 + 5}%` }}
                  />
                ))}
              </div>
            ...
            <div className="flex items-center justify-between text-[12px]">
              <span className="text-white/60">Avbl</span>
              <span className="flex items-center gap-1">
                0.00 USDT
                <span className="h-4 w-4 rounded-full border border-[#f5deb3]/60 text-[#f5deb3] flex items-center justify-center">
            ...
            <div className="space-y-1.5 pt-1">
              {["TP/SL", "Hidden Order"].map((l) => (
                <label key={l} className="flex items-center gap-2 text-[12px] text-white/70">
                  <span className="h-3.5 w-3.5 rounded-sm border border-white/30" />
                  <span className="border-b border-dashed border-white/20">{l}</span>
                </label>
              ))}
              <div className="flex items-center justify-between text-[12px] text-white/70">
                <label className="flex items-center gap-2">
                  <span className="h-3.5 w-3.5 rounded-sm border border-white/30" />
                  <span className="border-b border-dashed border-white/20">Reduce-Only</span>
                </label>
                <span className="flex items-center gap-1">
            ...
            <button className="w-full rounded-full bg-[#f5deb3] text-black py-2.5 text-[14px] font-medium mt-1">
              Connect
            </button>
          </div>
        </div>
      ...
      <section className="mx-3 mt-3 rounded-xl bg-[#111111] border border-white/5">
            ...
            {tabs.map((t) => (
              <button
                ...
                className={`py-3 text-[13px] whitespace-nowrap ${
                  tab === t ? "text-white border-b-2 border-white -mb-px" : "text-white/50"
                }`}
              >
                {t}
              </button>
        ...
        <div className="flex flex-col items-center justify-center py-12 gap-3">
          <div className="h-14 w-14 rounded-lg bg-white/5 flex items-center justify-center relative">
            <Link2 className="h-6 w-6 text-white/40" />
            <Search className="h-3.5 w-3.5 text-white/60 absolute bottom-2 right-2" />
          </div>
          <div className="text-white/60 text-[13px]">Please connect a wallet first</div>
        </div>
      </section>
      ...
      <div className="mt-6 flex justify-center">
        <div className="flex items-center gap-1 bg-[#111] border border-white/10 rounded-full p-1">
          <button className="h-9 w-16 rounded-full bg-white/5 flex items-center justify-center">
            <FileText className="h-4 w-4 text-white/80" />
          </button>
          <button className="h-9 w-16 rounded-full flex items-center justify-center">
            <CandlestickChart className="h-4 w-4 text-white/60" />
          </button>
        </div>
      </div>
      ...
      <div className="mt-6 mx-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-white/5 rounded-full px-4 py-2 text-[13px] text-white/80">
...
function BookRow({ row, side }: { row: Row; side: "ask" | "bid" }) {
  ...
  const bar = side === "ask" ? "bg-[#f6465d]/15" : "bg-[#0ecb81]/15";
  return (
    <div className="relative flex items-center justify-between text-[12px] leading-5">
      ...
      <span className="relative z-10 text-white/85">{row.size}</span>
    </div>
  );
}
...
function Line({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-white/50 border-b border-dashed border-white/15">{label}</span>
